...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn = (Button) findViewById(R.id.btn2);//找控件
        ...
        btn.setOnClickListener(new View.OnClickListener() {//点击事件
            ...
            public void onClick(View v) {
                ...
                //调用EcentBus里面的post（bean类的有参构造）方法，把需要的值存进bean包类中，然
//                而当我们点击这个按钮调用这个方法，就会直接响应第一个Activity里面的
//                onEventMainThread方法，当我们按返回键的时候就会看到文本已经设置上去了。
//                onEventMainThread
                String path1 = provinceSpinner.getSelectedItem().toString();
                String path2 = citySpinner.getSelectedItem().toString();
                String path3 = countySpinner.getSelectedItem().toString();
                EventBus.getDefault().post(
                        new FirstEvent(path1+path2+path3));
                finish();
            }
        });
    }
    ...
    private void setSpinner()
    {
        ...
        //绑定适配器和值
        provinceAdapter = new ArrayAdapter<String>(Main2Activity.this,
                android.R.layout.simple_spinner_item, province);
        provinceSpinner.setAdapter(provinceAdapter);
        provinceSpinner.setSelection(3,true);  //设置默认选中项，此处为默认选中第4个值

        cityAdapter = new ArrayAdapter<String>(Main2Activity.this,
                android.R.layout.simple_spinner_item, city[3]);
        citySpinner.setAdapter(cityAdapter);
        citySpinner.setSelection(0,true);  //默认选中第0个

        countyAdapter = new ArrayAdapter<String>(Main2Activity.this,
